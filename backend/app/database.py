...
async def _run_migrations(connection) -> None:
    ...
    """))
    await connection.execute(text(
        "CREATE INDEX IF NOT EXISTS ix_review_forward_outcomes_ledger_id "
        "ON review_forward_outcomes(ledger_id)"
    ))
    await connection.execute(text(
        "CREATE INDEX IF NOT EXISTS ix_review_forward_outcomes_symbol "
        "ON review_forward_outcomes(symbol)"
    ))
